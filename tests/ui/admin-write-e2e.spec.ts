...
test("admin login -> write publish -> public detail visible", async ({
  ...
  await page.getByLabel("태그 (콤마 구분)").fill("admin, playwright");

  const editor = page.getByPlaceholder("마크다운 본문을 입력하세요");
  await editor.fill("## 관리자 E2E\n\n본문 내용");
...
test("admin write page edits existing post", async ({ page, request }) => {
  const seed = Date.now();
  const created = await insertPostDirect(request, {
    title: `UI-ADMIN-EDIT-BEFORE-${seed}`,
    content: "수정 전 본문",
    tags: ["before"],
    status: "published",
    sourceUrl: `https://step9.test/ui/edit/${seed}`,
  });

  await authenticateAdminSession(page, {
    nextPath: `/admin/write?id=${created.id}`,
  });
  ...
  const updatedTitle = `UI-ADMIN-EDIT-AFTER-${seed}`;
