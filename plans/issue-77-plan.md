...
## 목표
- [x] 홈 링크를 1개로 통일해 중복 UI를 제거한다.
- [x] 타이틀(홈 링크) 클릭 동작을 정의한다.
- [x] 현재 경로가 `/`인 경우에도 클릭 시 "홈으로 돌아감" 동작이 체감되도록 처리한다.
- [x] 접근성/키보드 내비게이션(포커스, 탭 이동)을 확인한다.

## 완료 기준
- [x] 헤더에 홈 링크가 1개만 존재한다.
- [x] (테스트 기준) 헤더에서 `a[href="/"]`가 1개다.
- [x] 현재 경로가 `/`에서 스크롤을 내린 뒤 타이틀(홈 링크)을 클릭하면 스크롤이 최상단으로 복귀한다.
- [x] 접근성: 타이틀(홈 링크)이 명확한 포커스 스타일을 가지며, 키보드 탭 이동이 자연스럽다.
- [x] Playwright UI 테스트(스크린샷/기능 assertion)가 추가 또는 갱신된다.
- [x] 회귀 게이트: `npm run test:all` 통과
