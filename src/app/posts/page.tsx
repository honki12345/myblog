...
export default async function PostsPage({ searchParams }: PageProps) {
  ...
  return (
    <main className="mx-auto flex w-full max-w-5xl flex-col gap-6 px-4 py-8 sm:px-6 lg:px-8">
      ...
      <section className="grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900">
          ...
          {(
            ...
            return (
              <a
                ...
                className="rounded-full border border-slate-300 px-3 py-1.5 text-sm text-slate-700 hover:bg-slate-100 aria-[current=page]:border-slate-900 aria-[current=page]:bg-slate-900 aria-[current=page]:text-white dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-800"
              >
                {tab.label}
              </a>
            );
        ...
        <form method="get" action="/posts" className="grid gap-3">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="grid gap-1 text-sm font-medium text-slate-700 dark:text-slate-200">
              ...
              <PostsSearchTypeahead
                ...
                className="h-10 rounded-lg border border-slate-300 bg-white px-3 text-sm text-slate-900 placeholder:text-slate-400 focus:border-slate-900 focus:outline-none dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100 dark:placeholder:text-slate-500 dark:focus:border-slate-300"
              />
            </div>
          ...
          <div className="flex flex-wrap items-center justify-end gap-2">
            ...
            <a
              ...
              className="inline-flex h-10 items-center rounded-lg border border-slate-300 bg-white px-4 text-sm font-medium text-slate-700 hover:bg-slate-100 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800"
            >
              초기화
            </a>
          </div>
      ...
      {posts.length === 0 ? (
        <section className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center dark:border-slate-600 dark:bg-slate-900">
      ...
      {totalPages > 1 ? (
        ...
        >
          <a
            ...
            className="rounded-md border border-slate-300 px-3 py-1.5 text-sm text-slate-700 aria-disabled:pointer-events-none aria-disabled:opacity-40 dark:border-slate-600 dark:text-slate-200"
          >
            이전
          </a>
          {paginationItems.map((item) => {
            ...
            return (
              <a
                ...
                className="rounded-md border border-slate-300 px-3 py-1.5 text-sm text-slate-700 aria-[current=page]:border-slate-900 aria-[current=page]:bg-slate-900 aria-[current=page]:text-white dark:border-slate-600 dark:text-slate-200"
              >
                {item.value}
              </a>
            );
