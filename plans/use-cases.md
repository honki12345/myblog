...
| UC-WIKI-002 | WIKI | 공개 위키 트리/경로 조회 + 내용/태그 검색 + 숨김/삭제 비노출 + limit 단독 호환/검색 레이스 방지 + 상위경로 목적지 라벨 UX | `scripts/test-step-11.mjs`, `tests/ui/accessibility.spec.ts`, `tests/ui/wiki-view.spec.ts`, `tests/ui/visual-regression.spec.ts` | Active |
