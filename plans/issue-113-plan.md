...
- 코멘트 `2838534444`(plans/issue-113-plan.md): `## 테스트 시나리오 매트릭스` 표 앞 공백 줄을 추가해 MD058(표 주변 공백) 경고가 재발하지 않도록 정리.
- 코멘트 `2838580337`(docs/codebase.md): Data/control flow의 관리자 관련 불릿 시작어를 분산하고 인증/관리 API 흐름을 하나로 묶어 스캔 가독성 개선.
- 검증: 수정 구간 수동 점검(표 분리/문장 시작어 중복 감소) 완료.
